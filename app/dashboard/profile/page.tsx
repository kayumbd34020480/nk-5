...
export default function ProfilePage() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setAvatarError("Image size must be less than 500KB");
      return;
    }
    ...
    try {
      ...
      cloudinaryFormData.append('upload_preset', 'skl_app_upload');
      cloudinaryFormData.append('folder', 'skl_app_avatars');
      cloudinaryFormData.append('public_id', `${user.uid}-${Date.now()}`);

      // Upload directly to Cloudinary
      const response = await fetch('https://api.cloudinary.com/v1_1/dfrtk7d4k/image/upload', {
        method: 'POST',
      ...
      console.log("[v0] Cloudinary upload successful. Image URL:", secureUrl);
          ...
          <CardContent className="p-4 sm:p-6">
            ...
            {editMode && (
              <p className="text-xs text-gray-500 mb-4 text-center">
                Click the camera icon to upload a profile photo (max 500KB)
              </p>
            )}
